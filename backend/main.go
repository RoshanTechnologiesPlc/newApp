...
func getNewsList(w http.ResponseWriter) {
...
func getNewsByID(w http.ResponseWriter, id int) {
	var n News
	err := pool.QueryRow(context.Background(), "SELECT id, title, COALESCE(content, ''), COALESCE(image_url, ''), publisher_url, created_at FROM news WHERE id = $1", id).Scan(&n.ID, &n.Title, &n.Content, &n.ImageURL, &n.PublisherURL, &n.CreatedAt)
	if err != nil {
...
func scrapeAndStore(item *gofeed.Item) {
	...
	var content string
	var imageURL string
	var realURL string = item.Link

	// Handle redirects (especially for Google News)
	...
	c.OnHTML("article", func(e *colly.HTMLElement) {
		if content == "" {
			// Try to get actual article text
	...
	c.OnHTML("meta[property='og:image']", func(e *colly.HTMLElement) {
		if imageURL == "" {
			val := e.Attr("content")
			if !strings.Contains(val, "google") { // Avoid Google logos
				imageURL = val
			}
		}
	})
	...
	_, err = pool.Exec(context.Background(),
		...
		item.Title, content, imageURL, realURL, time.Now())
	if err != nil {
